...
# ============================================================
# COMANDOS FIXOS (execução direta, sem chamar o LLM)
# ============================================================

def cmd_status():
    """Resumo geral: RAM, disco, temperatura, containers"""
    ram = tool_netdata_metrics("ram")
    disk = tool_netdata_metrics("disk")
    temp = tool_netdata_metrics("temperature")
    containers = tool_docker_ps()
    # Formata a saída para ficar legível (já que as funções retornam JSON ou texto)
    return f"📊 RESUMO GERAL\n\n{ram}\n\n{disk}\n\n{temp}\n\n📦 CONTAINERS:\n{containers}"

def cmd_memoria():
    return tool_netdata_metrics("ram")

def cmd_temperatura():
    return tool_netdata_metrics("temperature")

def cmd_disco():
    return tool_netdata_metrics("disk")

def cmd_containers():
    return tool_docker_ps()

def cmd_logs():
    # Tenta pegar logs do container homeassistant (ajuste o nome se necessário)
    return tool_docker_logs("homeassistant", 50)

def cmd_ha():
    containers = tool_docker_ps()
    if "homeassistant" in containers and "Up" in containers:
        return "🏠 Home Assistant está rodando.\n\n" + tool_ha_states()[:500]
    return "Home Assistant não está rodando."

def cmd_entidades():
    return tool_ha_states()

def cmd_reiniciar(container=None):
    if not container:
        return "⚠️ Uso: /reiniciar <nome_do_container>\nEx: /reiniciar homeassistant"
    return tool_docker_restart(container)
...
def cmd_ajuda():
    return """📋 Comandos disponíveis (execução direta e rápida):

/status       - Resumo geral (RAM, disco, temperatura, containers)
/containers   - Lista todos os containers e status
/memoria      - Uso de RAM atual
/temperatura  - Temperatura da CPU
/disco        - Espaço disponível no cartão SD
/logs         - Últimos 50 logs do Home Assistant
/ha           - Status do Home Assistant
/entidades    - Lista entidades do Home Assistant
/reiniciar    - Reinicia um container (ex: /reiniciar homeassistant)
/limpar       - Limpa o histórico da conversa (apenas para o agente)
/ajuda        - Esta mensagem

Para perguntas livres, apenas escreva sua dúvida (ex: "qual a carga média?")."""

# Mapeamento dos comandos para as funções
FIXED_COMMANDS = {
    "/status": cmd_status,
    ...
    "/logs": cmd_logs,
    "/ha": cmd_ha,
    "/entidades": cmd_entidades,
    "/reiniciar": cmd_reiniciar,
    "/limpar": cmd_limpar,
    "/ajuda": cmd_ajuda,
}

def handle_fixed_command(cmd_text):
    parts = cmd_text.strip().split()
    cmd = parts[0].lower()
    if cmd in FIXED_COMMANDS:
        func = FIXED_COMMANDS[cmd]
        # Se for /reiniciar, passa o argumento se existir
        if cmd == "/reiniciar":
            arg = parts[1] if len(parts) > 1 else None
            return func(arg)
        else:
            return func()
    return None  # comando não reconhecido como fixo
...
def handle(message):
    ...
    # Tenta executar comando fixo primeiro
    resposta = handle_fixed_command(text)
    if resposta:
        bot.reply_to(message, resposta)
        return

    # Se não é comando fixo, usa o agente LLM
    status_msg = bot.reply_to(message, "🧠 Analisando...")
    try:
        answer, steps = run_agent(text)
        final = ("\n".join(steps) + "\n\n" if steps else "") + answer
        if len(final) > 4000:
            final = final[:3900] + "\n\n[Truncado]"
        bot.edit_message_text(final, chat_id=message.chat.id, message_id=status_msg.message_id)
    except Exception:
        err = traceback.format_exc()
        print(err)
        try:
            bot.edit_message_text(f"Erro:\n{err[:3000]}", chat_id=message.chat.id, message_id=status_msg.message_id)
        except:
            pass
